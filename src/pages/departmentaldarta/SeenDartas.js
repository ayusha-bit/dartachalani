...
const SeenDartas = () => {
  ...
  const dartaData = [
    {
      ...
      dartaId: 'D001',
      dartaNumber: 'DN1001',
      chalaniOrganizationName: 'Office A',
      chalaniSubject: 'Budget Proposal',
      chalaniNepaliDate: '2082-03-01',
      chalaniNumber: 'CHL1001',
      securityCategory: 'Confidential',
      urgency: 'High',
      chalaniFiscalYear: '2081/82',
      documentUrl: 'https://example.com/doc1.pdf',
      details: 'Complete details of Darta ID D001 from Office A.'
    },
    {
      id: 2,
      chalaniId: 'CHL002',
      dartaId: 'D002',
      dartaNumber: 'DN1002',
      chalaniOrganizationName: 'Office B',
      chalaniSubject: 'Annual Report',
      chalaniNepaliDate: '2082-03-02',
      chalaniNumber: 'CHL1002',
      securityCategory: 'General',
      urgency: 'Medium',
      chalaniFiscalYear: '2081/82',
      documentUrl: 'https://example.com/doc2.pdf',
      details: 'Details of Annual Report from Office B.'
    }
  ];

  const tabs = {
    received: dartaData,
    departmental: dartaData.slice(0, 1),
    personal: []
  };
  ...
  const headers = ['ID', 'Chalani ID', 'Darta ID', 'Darta Number', 'Chalani Organization Name', 'Chalani Subject', 'Chalani Nepali Date', 'Chalani Number', 'Security Category', 'Urgency', 'Chalani Fiscal Year', 'Action'];
  ...
  return (
    ...
    <div style={{ padding: '30px', backgroundColor: '#f9f9f9', minHeight: '100vh' }}>
      <h1>Received Dartas</h1>
      
        <Link to="/" style={{ display: 'inline-block', marginBottom: '20px' }}>&larr; Back to Dashboard</Link>

      <div style={{ marginBottom: '20px' }}>
        {['received', 'departmental', 'personal'].map(tab => (
          <button
            key={tab}
          ...
          >
            {tab === 'received' ? 'Received Darta' : tab === 'departmental' ? 'New Departmental Darta' : 'New Personal Darta'}
          </button>
        ))}
      </div>

      {tabs[activeTab].length === 0 ? (
        <p>No darta data available to display.</p>
      ) : (
          ...
          <tbody>
            {tabs[activeTab].map(darta => (
              <tr key={darta.id}>
                ...
                <td style={tdStyle}>
                  <button onClick={() => handleViewDocument(darta.documentUrl)} style={actionButton}>View Document</button>
                  <button onClick={() => handleView(darta)} style={actionButton}>View</button>
                </td>
              </tr>
            ))}
          </tbody>
